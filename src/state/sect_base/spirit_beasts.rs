...
impl SectBaseState {
    pub(super) fn draw_spirit_beasts_view(
        ...
        draw_text(&format!("Owned: {}", spirit_beasts.len()), rect.x + 20.0, header_y, FONT_HEADER_SIZE, TEXT_HIGHLIGHT);
        ...
        for (i, beast) in spirit_beasts.iter().enumerate() {
            ...
            y += 38.0;
            if y > list_rect.y + list_rect.h - 40.0 { break; }
        }
        ...
        if draw_button(Rect::new(rect.x + rect.w - 120.0, rect.y + 10.0, 100.0, 30.0), "Back", false) {
    ...
    pub(super) fn draw_beast_equip_modal(
        ...
        for item in items {
            ...
            y += 36.0;
            if y > modal_y + modal_h - 40.0 { break; }
        }
