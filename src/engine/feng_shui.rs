...
pub fn update_feng_shui_map(grid: &mut Grid, buildings: &mut Vec<Building>) {
    ...
    for building in buildings.iter() {
        if building.element == Element::None { continue; }
        ...
        for dy in -radius..=radius {
            for dx in -radius..=radius {
                ...
                if let Some(tile) = grid.get_tile_mut(tx, ty) {
                    ...
                    if decay > 0.0 {
                        let strength = *tile.element_strength.get(&building.element).unwrap_or(&0.0);
                        // Basic Propagation
                        tile.element_strength.insert(building.element.clone(), strength + (1.0 * decay));
                        ...
                        if suppressed_element != Element::None {
                             let target_strength = *tile.element_strength.get(&suppressed_element).unwrap_or(&0.0);
                             if target_strength > 0.0 {
                                 let reduction = 0.5 * decay; // Example dampening factor
                                 let new_strength = (target_strength - reduction).max(0.0);
                                 tile.element_strength.insert(suppressed_element, new_strength);
                             }
                        }
                    }
                }
