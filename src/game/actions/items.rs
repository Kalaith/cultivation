...
impl Game {
    pub(in crate::game) fn handle_use_item(&mut self, item_id: String, disciple_idx: usize) {
        ...
        let Some(item) = self.data.items.get(&item_id).cloned() else { return };
    ...
    pub(in crate::game) fn handle_unequip_item(&mut self, slot: crate::data::items::EquipmentSlot, disciple_idx: usize) {
        let Some(disciple) = self.disciples.get_mut(disciple_idx) else { return };
        let Some(item_id) = disciple.equipment.get(&slot).cloned() else { return };
        let Some(item) = self.data.items.get(&item_id) else { return };
    ...
    pub(in crate::game) fn handle_repair_item(&mut self, item_id: String, disciple_idx: usize) {
        let Some(disciple) = self.disciples.get(disciple_idx) else { return };
